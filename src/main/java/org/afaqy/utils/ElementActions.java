...
public class ElementActions {
    ...
    @Step("Selecting value: {value} from dropdown: {locator}")
    public static void selectFromDropdown(WebDriver driver, By locator, String value) {
        try {
            Waits.waitForElementVisible(driver, locator);

            WebElement dropdownElement = driver.findElement(locator);
            Scrolling.scrollToElement(driver, dropdownElement);

            Select dropdown = new Select(dropdownElement);
            dropdown.selectByVisibleText(value);

            LogsUtil.info("✅ Selected value: " + value + " from dropdown: " + locator);

        } catch (NoSuchElementException e) {
            LogsUtil.error("❌ Value '" + value + "' not found in dropdown: " + locator);
            throw e;

        } catch (Exception e) {
            LogsUtil.error("❌ Failed to select value '" + value + "' from dropdown: " + locator + ". Error: " + e.getMessage());
            throw e;
        }
    }
